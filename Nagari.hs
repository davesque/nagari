...
errMessage :: String -> ParserState -> String
errMessage msg ps = 
    "at " ++
    "line " ++ show (lineNr ps) ++", " ++
    "column " ++ show (colNr ps) ++ ": " ++
    msg

addMessage :: String -> ParserState -> ParserState
addMessage msg ps = ps { errors = errors ps ++ [errMessage msg ps] }
...
consumeErr x msg = Parser $ \ps -> (Just x, addMessage msg ps)

emptyOk :: Parser a
emptyOk =          Parser $ \ps -> (Nothing, ps)

emptyErr :: String -> Parser a
emptyErr msg =     Parser $ \ps -> (Nothing, addMessage msg ps)
...
lookAhead = Parser $ \ps -> case unParsed ps of
    []  -> (Nothing, ps)
    y:_ -> (Just y, ps)
...
{-takeAll' p = Parser $ \ps ->-}
    ...
    {-case x of-}
        {-Nothing -> (Nothing, ps)-}
        {-Just x' -> (Just x:xsFromOtherSuccesses, psFromLastSuccess)-}

{-takeAll :: Parser a -> Parser [a]-}
{-takeAll p = Parser $ \ps ->-}
    {-let (m, ps') = runParser p ps-}
        {-(m', ps'') = runParser p ps'-}
    {-in case m of-}
        {-Nothing -> (Nothing, ps)-}
        {-Just x  -> (Just (x:takeAll p ps'), ps')-}
...
takeWhile p = Parser $ \ps ->
    ...
    in case xs of
        ...
        _  -> let (xsInit, xsTail) = span p xs
              in if null xsInit
                 then (Nothing, ps)
                 else (Just xsInit, ps { unParsed = xsTail })
...
takeUntil s = Parser $ \ps ->
    ...
    in case findIn s xs of
        ...
        Just i  -> let (xsInit, xsTail) = splitAt i xs
                   in (Just xsInit, ps { unParsed = xsTail })
